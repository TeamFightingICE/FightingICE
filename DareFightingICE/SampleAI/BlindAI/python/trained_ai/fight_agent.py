...
import time
import torch
from torch_model import FeedForwardActor, Actor
from torch_encoder import RawEncoder, FFTEncoder, MelSpecEncoder, SampleEncoder
...
class SoundAgent:
    def __init__(self, gateway, **kwargs):
        ...
        self.device = 'cpu'
        if not self.rnn:
            self.actor = FeedForwardActor(STATE_DIM[1][self.encoder], 512, 1, get_sound_encoder(self.encoder))
        else:
            self.actor = Actor(STATE_DIM[1][self.encoder], 512, 1, get_sound_encoder(self.encoder))
        self.load_actor()
        if self.rnn:
    ...
    def processing(self):
        ...
        if self.just_inited:
            ...
            # action_dist = self.actor(state.unsqueeze(0).to(self.device), terminal=torch.tensor(0).float())
    ...
    def getAudioData(self, audio_data):
        ...
        # print('raw audio memory', self.raw_audio_memory.shape)

    def load_actor(self):
        # CHECKPOINT_PATH = 'model/{}/'.format(self.encoder)
        actor_state_dict = torch.load(self.path + "\\actor.pt", map_location=torch.device(self.device))
        # critic_state_dict = torch.load(CHECKPOINT_PATH + "critic.pt", map_location=torch.device(self.device))
